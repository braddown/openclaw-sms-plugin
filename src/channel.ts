...
import {
  ...
  resolveSmsAccountSync,
  checkSmsAuthenticated,
} from "./accounts.js";
...
export const smsPlugin: ChannelPlugin<ResolvedSmsAccount> = {
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveSmsAccountSync({ cfg, accountId }).config.allowFrom ?? []).map((entry) =>
        String(entry)
      ),

    formatAllowFrom: ({ allowFrom }) =>
      formatAllowFromLowercase({ allowFrom, stripPrefixRe: /^(sms|phone):/i }),
  },
