import type { ResolvedSmsAccount, KudosityApiMessage, KudosityApiResponse, SendResult } from "./types.js";
...
export async function sendSmsMessage(
  ...
  try {
    const message: KudosityApiMessage = {
      to: to.trim(),
    ...
    try {
      ...
      const result: KudosityApiResponse = await response.json();
      ...
      if (result.status === "blocked") {
        return {
          ...
          messageId: result.id,
          error: new Error(
            `Message blocked by safety engine: ${result.safety?.denied_reason || "Safety check failed"}`
          ),
        };
      }
...
export function chunkSmsText(text: string, maxLength: number = 1600): string[] {
